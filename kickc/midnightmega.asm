...
.byte $00, $00, $00                                     // 
  ...
  .const OFFSET_STRUCT_DIRENT_LENGTH = $40
  .const OFFSET_STRUCT_DIRENT_SFN = $41
  .const OFFSET_STRUCT_HYPPOFILENAME_SFN = $41
  ...
  .label readdir_dirent = $6000
  // needs be at page frame
...
*/
  ...
  .label conio_cursor_y = $5b
  // The current text cursor line start
  .label conio_line_text = $5c
  // The current color cursor line start
  .label conio_line_color = $5e
  ...
  .label ptrMiniOffs = $45
  // = "";
  .label offsCurrIdx = $61
  .label flagCurrSec = $62
  .label datNextTrk = $52
  .label datNextSec = $53
  .label workside = $55
  .label i = $3e
...
*/
  ...
  __b11:
    ...
    sta.z printf_str.s+1
    jsr printf_str
    lda #<hyppofn+OFFSET_STRUCT_HYPPOFILENAME_SFN
    sta.z strlen.str
    lda #>hyppofn+OFFSET_STRUCT_HYPPOFILENAME_SFN
    sta.z strlen.str+1
    jsr strlen
...
// void printf_str(__zp(7) void (*putc)(char), __zp($a) const char *s)
printf_str: {
    .label s = $a
    .label putc = 7
  __b1:
...
.segment Code
// __register(A) char hyppo_selectdrive(__zp($4f) volatile char nb)
hyppo_selectdrive: {
    .label nb = $4f
    ldx nb
	lda #$06
...
// void printf_uint(void (*putc)(char), __zp(2) unsigned int uvalue, __zp($16) char format_min_length, char format_justify_left, char format_sign_always, char format_zero_padding, char format_upper_case, char format_radix)
printf_uint: {
    .label uvalue = 2
...
// __register(A) char hyppo_readdir(__zp($42) volatile char filedescriptor)
hyppo_readdir: {
    .label filedescriptor = $42
...
error:
    ...
    lda #0
    ldy readdir_dirent+OFFSET_STRUCT_DIRENT_LENGTH
    sta readdir_dirent,y
    // put str terminate null
    lda.z retval
    rts
}
// __register(A) char hyppo_closedir(__zp($50) volatile char filedescriptor)
hyppo_closedir: {
    .label filedescriptor = $50
    ldx filedescriptor
    lda #$16
	sta HTRAP00
	clv
...
getsfn: {
    ...
    sta.z i+1
  __b1:
    lda #<readdir_dirent+OFFSET_STRUCT_DIRENT_SFN
    clc
    adc.z i
    sta.z __10
    lda #>readdir_dirent+OFFSET_STRUCT_DIRENT_SFN
  ...
  __b3:
    lda #'.'
    sta hyppofn+OFFSET_STRUCT_HYPPOFILENAME_SFN,x
    inx
    lda #<8
    sta.z i1
    lda #>8
...
printf_string: {
    .label putc = cputc
    .label str = hyppofn+OFFSET_STRUCT_HYPPOFILENAME_SFN
    lda #<putc
    sta.z printf_str.putc
    lda #>putc
    sta.z printf_str.putc+1
...
// __zp(2) unsigned int strlen(__zp(7) char *str)
strlen: {
    .label len = 2
    .label str = 7
    .label return = 2
...
// void utoa(__zp(2) unsigned int value, __zp($10) char *buffer, __register(A) char radix)
utoa: {
    .label digit_value = 4
    .label buffer = $10
    .label value = 2
...
// void printf_number_buffer(__zp($10) void (*putc)(char), __zp($20) char buffer_sign, char *buffer_digits, __register(X) char format_min_length, __zp(6) char format_justify_left, char format_sign_always, __zp($1a) char format_zero_padding, __zp($3c) char format_upper_case, char format_radix)
...
// bytes buffer:
// __register(X) char GetWholeSector(__zp($10) struct structBAM *entry, char drive, __register(X) char track, __register(Y) char sector)
GetWholeSector: {
    .label i = $25
    .label __5 = $1e
    .label entry = $10
...
// char PutWholeSector(__zp(7) struct structBAM *entry, __zp($16) char side, char drive, __register(X) char track, __register(Z) char sector)
PutWholeSector: {
    .label i = $27
    .label __6 = $1c
    .label entry = 7
...
// void BAMSectorUpdate(__zp($1c) struct structBAM *BAMsector, __zp(6) char track, __register(A) char sector, char set)
BAMSectorUpdate: {
    .const bitshifter = 1
    .label __60 = $39
    .label __61 = $31
...
// void printf_uchar(void (*putc)(char), __register(A) char uvalue, __zp($3c) char format_min_length, char format_justify_left, char format_sign_always, char format_zero_padding, char format_upper_case, char format_radix)
printf_uchar: {
    .label format_min_length = $3c
    // Handle any sign
    ldz #0
    stz printf_buffer
    sta.z uctoa.value
...
// void * memcpy(__zp($35) void *destination, __zp($2a) void *source, unsigned int num)
memcpy: {
    .label src_end = $40
    .label dst = $35
    .label src = $2a
...
// void * memset(__zp($2a) void *str, __register(Z) char c, unsigned int num)
memset: {
    .label end = $35
    .label dst = $2a
    .label str = $2a
...
// __zp(2) unsigned int utoa_append(__zp($18) char *buffer, __zp(2) unsigned int value, __zp(4) unsigned int sub)
utoa_append: {
    .label buffer = $18
    .label value = 2
    .label sub = 4
...
// void printf_padding(__zp(7) void (*putc)(char), __zp($17) char pad, __zp($16) char length)
printf_padding: {
    .label i = 9
    .label putc = 7
    .label length = $16
...
// __register(X) char ReadSector(__zp($2c) volatile char drive, __zp($33) volatile char track, __zp($3d) volatile char sector)
ReadSector: {
    .label drive = $2c
    .label track = $33
    .label sector = $3d
...
// char WriteSector(__zp($29) volatile char drive, __zp($3f) volatile char track, __zp($34) volatile char sector)
WriteSector: {
    .label drive = $29
    .label track = $3f
    .label sector = $34
...
// - buffer : receives the string representing the number and zero-termination.
// - radix : The radix to convert the number to (from the enum RADIX)
// void uctoa(__zp($1b) char value, __zp(7) char *buffer, char radix)
uctoa: {
    .label digit_value = 6
    .label buffer = 7
    .label digit = $20
...
// - buffer : receives the string representing the number and zero-termination.
// - radix : The radix to convert the number to (from the enum RADIX)
// void ultoa(__zp($c) unsigned long value, __zp(2) char *buffer, char radix)
ultoa: {
    .const max_digits = 8
...
//        (For decimal the subs used are 10000, 1000, 100, 10, 1)
// returns : the value reduced by sub * digit so that it is less than sub.
// __register(A) char uctoa_append(__zp($21) char *buffer, __register(Z) char value, __zp(6) char sub)
uctoa_append: {
    .label buffer = $21
    .label sub = 6
    ldx #0
...
//        (For decimal the subs used are 10000, 1000, 100, 10, 1)
// returns : the value reduced by sub * digit so that it is less than sub.
// __zp($c) unsigned long ultoa_append(__zp($23) char *buffer, __zp($c) unsigned long value, __zp($12) unsigned long sub)
ultoa_append: {
    .label buffer = $23
    .label value = $c
    .label sub = $12
